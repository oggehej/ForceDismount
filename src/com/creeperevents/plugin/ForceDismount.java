			...
			if(args.length != 0)
			{
				...
				if(player != null)
				{
					if(!player.hasPermission("forcedismount.exempt"))
					{
						...
						else sender.sendMessage(ChatColor.RED + player.getDisplayName() + " isn't riding a vehicle!");
					}
					else sender.sendMessage(ChatColor.RED + "You can't force " + player.getDisplayName() + " to dismount a vehicle!");
				}
				else sender.sendMessage(ChatColor.RED + "Couldn't find that player!");
			}
			else sender.sendMessage(ChatColor.RED + "You have to enter a name!");
